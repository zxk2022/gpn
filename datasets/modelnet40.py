...
class ModelNet40Dataset(InMemoryDataset):
    ...
    def raw_file_names(self) -> List[str]:
        ...
        return [osp.join(self.raw_dir, f'modelnet40_{"train" if self.train else "test"}_1024pts_fps_pcd', folder) 
                for folder in os.listdir(osp.join(self.raw_dir, f'modelnet40_{"train" if self.train else "test"}_1024pts_fps_pcd')) 
                if osp.isdir(osp.join(self.raw_dir, f'modelnet40_{"train" if self.train else "test"}_1024pts_fps_pcd', folder))]
