...
__global__ void message_passing_sum_kernel(
    ...
    const int batch_size, const int num_graphs_per_batch, const int num_nodes, const int feature_channels, const int num_edges_max)
{
    ...
    for (int node_idx = 0; node_idx < num_nodes; ++node_idx) {
        ...
        for (int c = 0; c < feature_channels; ++c) {
            node_output[c] = input[batch_idx * num_graphs_per_batch * num_nodes * feature_channels + graph_idx * num_nodes * feature_channels + node_idx * feature_channels + c];
        }
                 ...
                 const int batch_size, const int num_graphs_per_batch, const int num_nodes, const int feature_channels, const int num_edges_max)
{
    ...
    // 启动核函数
    message_passing_sum_kernel<<<blocks, threads>>>(output, input, edge_index, batch_size, num_graphs_per_batch, num_nodes, feature_channels, num_edges_max);
...
__global__ void message_passing_sum_grad_kernel(
    ...
    const int batch_size, const int num_graphs_per_batch, const int num_nodes, const int feature_channels, const int num_edges_max)
{
    ...
    for (int node_idx = 0; node_idx < num_nodes; ++node_idx) {
        ...
        for (int c = 0; c < feature_channels; ++c) {
            d_node_input[c] = d_output[batch_idx * num_graphs_per_batch * num_nodes * feature_channels + graph_idx * num_nodes * feature_channels + node_idx * feature_channels + c];  // 使用输出梯度初始化输入梯度
        }
                  ...
                  const int batch_size, const int num_graphs_per_batch, const int num_nodes, const int feature_channels, const int num_edges_max)
{
    ...
    // 启动核函数
    message_passing_sum_grad_kernel<<<blocks, threads>>>(grad_input, grad_output, edge_index, batch_size, num_graphs_per_batch, num_nodes, feature_channels, num_edges_max);
