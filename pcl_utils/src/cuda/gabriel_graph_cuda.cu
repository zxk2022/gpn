...
__global__ void build_gabriel_graph_kernel(const PointXYZ* points, int num_points, Edge* edges, int* num_edges) {
    ...
    PointXYZ mid_point = (point_i + point_j) / 2.0f;
    ...
    for (int k = 0; k < num_points; ++k) {
        if (k != i && k != j) {
            ...
            float dist_mid_k = (mid_point - point_k).norm();
            if (dist_mid_k <= dist_ij / 2.0f) {
...
void build_gabriel_graph_kernel_launcher(const pcl::PointCloud<pcl::PointXYZ>& points, std::vector<pcl_utils::Edge>& edges) {
    ...
    // 启动CUDA内核
    build_gabriel_graph_kernel<<<gridSize, blockSize>>>(thrust::raw_pointer_cast(d_points.data()), num_points, thrust::raw_pointer_cast(d_edges.data()), d_num_edges);
